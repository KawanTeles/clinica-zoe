-- Migração inicial: copiar telefone -> whatsapp quando vazio
UPDATE public.profissionais SET whatsapp = telefone
 WHERE (whatsapp IS NULL OR btrim(whatsapp) = '') AND telefone IS NOT NULL AND btrim(telefone) <> '';

UPDATE public.pacientes SET whatsapp = telefone
 WHERE (whatsapp IS NULL OR btrim(whatsapp) = '') AND telefone IS NOT NULL AND btrim(telefone) <> '';

-- Notificações passam a usar exclusivamente o campo whatsapp
...
DECLARE
  ...
  r record;
BEGIN
  SELECT p.user_id, p.nome, NULLIF(btrim(p.whatsapp),''), p.email, e.nome
  ...
  SELECT pa.nome, pa.user_id, NULLIF(btrim(pa.whatsapp),''), pa.email
...
BEGIN
  IF TG_OP = 'UPDATE' AND NEW.status_pagamento = 'PAGO' AND OLD.status_pagamento IS DISTINCT FROM 'PAGO' THEN
    SELECT pa.user_id, pa.nome, NULLIF(btrim(pa.whatsapp),''), pa.email
